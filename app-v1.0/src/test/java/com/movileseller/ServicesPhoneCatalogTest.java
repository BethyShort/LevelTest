...
public class ServicesPhoneCatalogTest {

	@Value("${phoneCatalogEndpoint}")
	private String catalogEndpoing;
	...
	public void isServiceRunning() {
		...
		HttpUriRequest request = new HttpGet(catalogEndpoing);
		try {
	...
	public void  serializationTest() throws ClientProtocolException, IOException{
		try {
		    HttpUriRequest request = new HttpGet(catalogEndpoing+"/"+phone);
		    HttpResponse response = HttpClientBuilder.create().build().execute( request );
		    PhoneDTO resource = retrieveResourceFromResponse(response, PhoneDTO.class);
		    assertThat( phone.getPhoneId(), Matchers.is( resource.getPhoneId()) );
		} catch (Exception ex) {
			fail("error executing isServiceRunning test. Exception catched");
		}
	}
